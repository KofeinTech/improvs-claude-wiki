# /improvs:write-tests -- Independent Test Generation
...
## When is /improvs:write-tests required?

| Task complexity | /improvs:write-tests required? |
|----------------|----------------|
| Trivial | No |
| Simple | Yes (checked by /improvs:finish) |
| Complex | Yes (checked by /improvs:finish) |

If you skip /improvs:write-tests, the PR will be flagged with a warning.

## Related

- [/improvs:finish](../workflow/finish.md) -- runs /improvs:write-tests automatically if tests are missing
- [/improvs:start](../workflow/start.md) -- sets task complexity that determines if /improvs:write-tests is required
